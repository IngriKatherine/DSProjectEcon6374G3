...
###### INITIAL CLEANING ALL STATES ######
lar2021 = read.csv("raw_data/2021_public_lar.csv", header = TRUE, nrows = 1000)
lar = read.csv("raw_data/2023_public_lar.csv", header = TRUE, nrows = 1000)

### Drop missings in state
lar2021 = lar2021[!is.na(lar2021$state_code), ]
### Keep only the variables we need for efficiency
varstokeep= c("activity_year", "state_code","county_code", 
...
lar2021=lar2021[varstokeep]

### Create variable that identifies Group 3 states
lar2021$g3 = ifelse(lar2021$state_code == "TX" | lar2021$state_code == "LA", 1, 0)

### Fix debt to income ratio variable
lar2021 <- lar2021 %>%
  mutate(
    # Step 1: numeric "minimum DTI" using gsub + first 2 digits
    dti_digits = gsub("[^0-9]", "", debt_to_income_ratio),                 # keep only digits
    ...
    debt_to_income_ratio = case_when(
      ...
      !is.na(dti_min) & dti_min >= 36 & dti_min < 50 ~ "36%-<50%",  # 36–49 go here
...
### Convert to factors and add levels to everything else
lar2021 <- lar2021 %>%
  mutate(
    #States Group3
    g3 = factor(
    ...
    # Construction Method (Property type stand-in)
...
for (v in names(var_labels)) {
  if (v %in% names(lar2021)) {
    attr(lar2021[[v]], "label") <- var_labels[[v]]
  }
}

###### TABLE G3 vs OTHER ######
summary_g3vsoth = lar2021 %>%
  group_by(g3) %>%
  summarise(
    median_loan_amount = median(loan_amount, na.rm = TRUE),
    average_interest_rate = mean(interest_rate, na.rm = TRUE)
  ) %>%
  select(g3, median_loan_amount, average_interest_rate)
print(summary_g3vsoth)

###### FILTER TO ONLY THE STATES WE NEED ######
g3lar2021 = lar2021 %>%filter(g3 == 1)
#Clean space
rm(lar2021)
...
###### SAVE FINAL DATASET ######
write_parquet(TXLA, "proc_data/g3lar.parquet")
