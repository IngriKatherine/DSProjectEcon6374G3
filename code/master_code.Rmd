...
# Correct short labels mapping: short = long
loan_labels <- c(
  "Conventional" = "Conventional (not insured or guaranteed by FHA VA RHS or FSA)",
  "FHA"          = "Federal Housing Administration insured (FHA)",
  "VA"           = "Veterans Affairs guaranteed (VA)",
  "USDA"         = "USDA RHS or FSA guaranteed"
)

plot_5b_data <- plot_5b_data %>%
  mutate(
    loan_type_short = fct_recode(loan_type, !!!loan_labels),
    loan_type_short = fct_relevel(loan_type_short, "Conventional", "FHA", "VA", "USDA")
  )
...
plot_5b <- ggplot(
  ...
  # REMOVED geom_text() to remove bar labels
  ...
  labs(
    ...
    caption = "Note: Loan types are Conventional, FHA, VA, USDA. \"Not Applicable\" is excluded from this plot.",
    x = "Loan Type",
...
```{r Q5c}
# Question 5(c): Average interest rate by year and Property Type (2021 & 2023 only)
table_5c <- g3lar %>% 
...
kable(table_5d,
      format = "markdown",
      caption = "Count of Loans by Action Taken and Derived Ethnicity") %>%
  kable_styling(full_width = FALSE)
...
The graph of the Action Taken by Occupancy is quite hard to read, so we included individual plots of each Occupancy to make it clearer.
```{r Q6dg}
ggplot(g3lar, aes(x = action_taken, fill = occupancy_type)) +
  ...
  scale_fill_manual(
    values = c(
      "Principal residence"   = "#E64B35",
      "Second residence"      = "#81F121",
      "Investment property"   = "#4DBBD5"
    )
  ) +
  scale_y_continuous(
...
Due to the large difference in Values, it was difficult to see certain variables on the bar chart, so we added separate bar charts for each occupancy type for better comparison.
...
for (yr in years_to_run) {
  ...
  cat("\n\n=========================================\n")
  cat("     CORRELATION MATRIX FOR YEAR", yr, "\n")
  cat("=========================================\n\n")

  # Filter dataset for selected year
  ds_year <- FREDLARTXLA_l %>%
    filter(year == yr)
  ...
  corrplot(
    ...
    addCoef.col = "black",
    number.cex = 0.7,
    main = paste("Correlation Matrix for Year", yr)
  )
  ...
  cat("\n\nHigh-Correlation Pairs (|corr| > 0.85) for Year ", yr, ":\n", sep = "")
...
for (yr in years_to_run) {
  ...
  cat("\n\n=========================================\n")
  cat(" CORRELATION WITH LOG LOAN AMOUNT PER CAPITA — YEAR", yr, "\n")
  cat("=========================================\n\n")
  ...
  # ----------------------------------------
  # Print header for barplot
  # ----------------------------------------
  cat("Barplot for Year ", yr, "\n\n", sep = "")
